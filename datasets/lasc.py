...
def load_lasc_slice_list(root_dir, split='train', slice_split=[4400, 4884]):
    """
...
def load_lasc_cls_labels(name_list):
    """
    Extract classification labels from filenames.
    Filename format: {name}_{label}.npy where label is 0 or 1
    ...
    for name in name_list:
        # Parse label from filename: lasc_XXX_YY_Z.npy -> Z is label
...
class LASCDataset(Dataset):
    """
    Base dataset class for LASC medical imaging dataset.
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.name_list = load_lasc_slice_list(root_dir, split, slice_split)
    ...
    def __getitem__(self, idx):
        ...
        # LASC has no segmentation labels - return dummy
...
class LASCClsDataset(LASCDataset):
    """
    ...
    def __init__(self,
        ...
        self.label_list = load_lasc_cls_labels(self.name_list)
...
class LASCClsValDataset(LASCDataset):
    """
    LASC validation dataset.

    Since LASC has no segmentation ground truth, this returns dummy
    segmentation labels (all zeros) for compatibility with the training
    loop. Segmentation metrics will NOT be meaningful.

    Focus validation on classification metrics (F1, accuracy, etc.).
    """
    def __init__(self,
                 root_dir=None,
                 split='val',
    ...
    def __getitem__(self, idx):
        ...
        # This is a placeholder - mIoU metrics will NOT be meaningful
